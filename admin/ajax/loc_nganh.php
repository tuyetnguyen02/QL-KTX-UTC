...
if(isset($_SESSION['useradmin'])){
    if(isset($_GET['giatri']) && isset($_GET['loc_khoa']) && isset($_GET['loc_gioitinh'])){//
        $giatri = $_GET['giatri'];
        $giatri = str_replace('\'', '\\\'', $giatri);
        $loc_khoa = $_GET['loc_khoa'];
        $loc_khoa = str_replace('\'', '\\\'', $loc_khoa);
        ...
        $sql_student = "SELECT s.*, c.*, r.*, rtype.*, sem.* FROM student s
                        ...
                        WHERE c.status = true AND sem.status = true AND s.major like '%$giatri%' ";
        if($loc_khoa != ''){
            $sql_student .= "AND s.classroom like '%$loc_khoa%'";
        }
        if($loc_gioitinh != ''){
